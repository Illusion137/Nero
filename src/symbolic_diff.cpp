...
std::string nero::ast_to_latex(const AST* ast) {
    if (!ast) return "0";
    using TT = nero::TokenType;

    switch (ast->token.type) {
        case TT::NUMERIC_LITERAL: {
            const auto& expr = std::get<nero::AST::ASTExpression>(ast->data);
            const auto* uv = std::get_if<nero::UnitValue>(&expr.value);
            if (!uv) return "0";
...
std::string nero::symbolic_diff_latex(const AST* ast, const std::string& var) {
    if (!ast) return "0";
    using TT = nero::TokenType;
